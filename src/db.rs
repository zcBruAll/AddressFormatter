...
impl SqlClient {
    ...
    pub fn ensure_address_table(&self, table: String) -> anyhow::Result<u64> {
        let sql = format!(
            r#"
        ...
        self.execute_non_query(&sql).map_err(Into::into)
    }
...
pub fn n_opt(v: Option<&str>) -> String {
    match v {
        Some(s) if !s.trim().is_empty() => format!("{}", sql_quote(s)),
        _ => "NULL".to_string(),
    }
}
